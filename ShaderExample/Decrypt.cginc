...
static const uint WOFFSET = 5;
static const uint HOFFSET = 5;
...
float4 DecryptTextureXXTEA(float2 uv, int m)
{
	...
	key[3] = ((uint)(key12) | (uint)(key13 << 8) | (uint)(key14 << 16) | (uint)(key15 << 24)) ^ (uint)(floor(idx / 4) * 4);
	//key make end
	//4idx
...
float4 DecryptTextureXXTEADXT(float2 uv, int m)
{
	...
	uint idx = (w * floor(y * h)) + floor(x * w);
